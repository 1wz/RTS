...
public class CommandButtonsPresenter : MonoBehaviour
{
    ...
    private void onButtonClick(ICommandExecutor commandExecutor)
    {
        ...
        {
            unitProducer.ExecuteSpecificCommand(_context.Inject(new
            ProduceUnitCommand()));
            return;
        }
        ...
        if (stopper != null)
        {
            stopper.ExecuteSpecificCommand(_context.Inject(new
            StopCommand()));
            return;
        }
        ...
        if (mover != null)
        {
            mover.ExecuteSpecificCommand(_context.Inject(new
            MoveCommand()));
            return;
        }
        ...
        if (patroller != null)
        {
            patroller.ExecuteSpecificCommand(_context.Inject(new
            PatrolCommand()));
            return;
        }
        throw new
        ApplicationException($"{nameof(CommandButtonsPresenter)}." +
        $"{nameof(onButtonClick)}: Unknown type of commands executor:" +
        $"{ commandExecutor.GetType().FullName }!");
}
}